...
const WorkflowEventProcessingModel = {
   ...
   */
  getByEventId: async (knex: Knex, tenant: string, eventId: string): Promise<IWorkflowEventProcessing[]> => {
    try {
      if (!tenant) {
   ...
   */
  getByExecutionId: async (knex: Knex, tenant: string, executionId: string): Promise<IWorkflowEventProcessing[]> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  getById: async (knex: Knex, tenant: string, processingId: string): Promise<IWorkflowEventProcessing | null> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  getLatestByEventId: async (knex: Knex, tenant: string, eventId: string): Promise<IWorkflowEventProcessing | null> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  create: async (knex: Knex, tenant: string, record: Omit<IWorkflowEventProcessing, 'processing_id' | 'created_at' | 'updated_at'>): Promise<Pick<IWorkflowEventProcessing, 'processing_id'>> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  update: async (knex: Knex, tenant: string, processingId: string, record: Partial<IWorkflowEventProcessing>): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  updateStatus: async (knex: Knex, tenant: string, processingId: string, status: WorkflowEventProcessingStatus, errorMessage?: string): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  markAsPublished: async (knex: Knex, tenant: string, processingId: string, workerId?: string): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  markAsProcessing: async (knex: Knex, tenant: string, processingId: string, workerId: string): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  markAsCompleted: async (knex: Knex, tenant: string, processingId: string): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  markAsFailed: async (knex: Knex, tenant: string, processingId: string, errorMessage: string): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
   ...
   */
  markAsRetrying: async (knex: Knex, tenant: string, processingId: string, workerId: string): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
  ...
  /**
   * Get all events that need to be processed across all tenants
   */
  getPendingEvents: async (limit: number = 100): Promise<IWorkflowEventProcessing[]> => {
    try {
      ...
      const records = await knex('workflow_event_processing')
        .where({ 
          status: 'pending'
        })
        .select('*')
  ...
  /**
   * Get all events that need to be retried across all tenants
   */
  getEventsToRetry: async (knex: Knex, limit: number = 100, maxAttempts: number = 3): Promise<IWorkflowEventProcessing[]> => {
    try {
      const records = await knex<IWorkflowEventProcessing>('workflow_event_processing')
        .where({ 
          status: 'failed'
   ...
   */
  delete: async (knex: Knex, tenant: string, processingId: string): Promise<void> => {
    try {
      if (!tenant) {
        throw new Error('Tenant not found');
      }
