            ...
            return knex('bucket_usage').insert([
                {
                    ...
                    bucket_plan_id: knex('bucket_plans').where({ tenant: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', plan_id: knex('billing_plans').where({ tenant: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', plan_name: 'Wonderland Basic' }).select('plan_id').first() }).select('bucket_plan_id').first(),
                ...
                {
                    tenant: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
                    bucket_plan_id: knex('bucket_plans').where({ tenant: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', plan_id: knex('billing_plans').where({ tenant: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', plan_name: 'Oz Premium' }).select('plan_id').first() }).select('bucket_plan_id').first(),
