import { createTenantKnex } from '@/lib/db';
import { IWorkflowExecution } from './workflowInterfaces';

/**
 * Model for workflow_executions table
 * Provides CRUD operations for workflow executions
 */
const WorkflowExecutionModel = {
  /**
   * Get all workflow executions
   */
  getAll: async (): Promise<IWorkflowExecution[]> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      const executions = await knex<IWorkflowExecution>('workflow_executions')
        .where({ tenant })
        .select('*');
      
      return executions;
    } catch (error) {
      console.error('Error getting all workflow executions:', error);
      throw error;
    }
  },

  /**
   * Get a workflow execution by ID
   */
  getById: async (executionId: string): Promise<IWorkflowExecution | null> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      const execution = await knex<IWorkflowExecution>('workflow_executions')
        .where({ 
          execution_id: executionId,
          tenant 
        })
        .first();
      
      return execution || null;
    } catch (error) {
      console.error(`Error getting workflow execution with id ${executionId}:`, error);
      throw error;
    }
  },

  /**
   * Create a new workflow execution
   */
  create: async (execution: Omit<IWorkflowExecution, 'execution_id' | 'created_at' | 'updated_at'>): Promise<Pick<IWorkflowExecution, 'execution_id'>> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      const [insertedExecution] = await knex<IWorkflowExecution>('workflow_executions')
        .insert({
          ...execution,
          tenant: tenant
        })
        .returning('execution_id');
      
      return { execution_id: insertedExecution.execution_id };
    } catch (error) {
      console.error('Error creating workflow execution:', error);
      throw error;
    }
  },

  /**
   * Update a workflow execution
   */
  update: async (executionId: string, execution: Partial<IWorkflowExecution>): Promise<void> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      await knex<IWorkflowExecution>('workflow_executions')
        .where({ 
          execution_id: executionId,
          tenant 
        })
        .update({
          ...execution,
          updated_at: new Date().toISOString()
        });
    } catch (error) {
      console.error(`Error updating workflow execution with id ${executionId}:`, error);
      throw error;
    }
  },

  /**
   * Delete a workflow execution
   */
  delete: async (executionId: string): Promise<void> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      await knex<IWorkflowExecution>('workflow_executions')
        .where({ 
          execution_id: executionId,
          tenant 
        })
        .del();
    } catch (error) {
      console.error(`Error deleting workflow execution with id ${executionId}:`, error);
      throw error;
    }
  },

  /**
   * Get workflow executions by workflow name
   */
  getByWorkflowName: async (workflowName: string): Promise<IWorkflowExecution[]> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      const executions = await knex<IWorkflowExecution>('workflow_executions')
        .where({ 
          workflow_name: workflowName,
          tenant 
        })
        .select('*');
      
      return executions;
    } catch (error) {
      console.error(`Error getting workflow executions for workflow ${workflowName}:`, error);
      throw error;
    }
  },

  /**
   * Get workflow executions by current state
   */
  getByState: async (state: string): Promise<IWorkflowExecution[]> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      const executions = await knex<IWorkflowExecution>('workflow_executions')
        .where({ 
          current_state: state,
          tenant 
        })
        .select('*');
      
      return executions;
    } catch (error) {
      console.error(`Error getting workflow executions in state ${state}:`, error);
      throw error;
    }
  },

  /**
   * Get workflow executions by status
   */
  getByStatus: async (status: string): Promise<IWorkflowExecution[]> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
      }
      
      const executions = await knex<IWorkflowExecution>('workflow_executions')
        .where({ 
          status,
          tenant 
        })
        .select('*');
      
      return executions;
    } catch (error) {
      console.error(`Error getting workflow executions with status ${status}:`, error);
      throw error;
    }
  }
};

export default WorkflowExecutionModel;