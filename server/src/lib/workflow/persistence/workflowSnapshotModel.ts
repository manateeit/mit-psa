...
const WorkflowSnapshotModel = {
   ...
   */
  getLatestByExecutionId: async (executionId: string): Promise<IWorkflowSnapshot | null> => {
    try {
   ...
   */
  getByVersion: async (executionId: string, version: number): Promise<IWorkflowSnapshot | null> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  create: async (snapshot: WorkflowStateSnapshot): Promise<void> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  pruneSnapshots: async (executionId: string, keepCount: number = 5): Promise<void> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  exists: async (executionId: string, version: number): Promise<boolean> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
