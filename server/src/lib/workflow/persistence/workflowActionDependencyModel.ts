...
const WorkflowActionDependencyModel = {
   ...
   */
  getDependenciesForAction: async (actionId: string): Promise<IWorkflowActionDependency[]> => {
    try {
   ...
   */
  getDependentsForAction: async (dependsOnId: string): Promise<IWorkflowActionDependency[]> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  getByExecutionId: async (executionId: string): Promise<IWorkflowActionDependency[]> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  getByEventId: async (eventId: string): Promise<IWorkflowActionDependency[]> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  getById: async (dependencyId: string): Promise<IWorkflowActionDependency | null> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  create: async (dependency: Omit<IWorkflowActionDependency, 'dependency_id' | 'created_at'>): Promise<Pick<IWorkflowActionDependency, 'dependency_id'>> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  createMany: async (dependencies: Omit<IWorkflowActionDependency, 'dependency_id' | 'created_at'>[]): Promise<void> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  deleteForAction: async (actionId: string): Promise<void> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
   ...
   */
  deleteForEvent: async (eventId: string): Promise<void> => {
    try {
      const { knex, tenant } = await createTenantKnex();
      if (!tenant) {
        throw new Error('Tenant not found');
