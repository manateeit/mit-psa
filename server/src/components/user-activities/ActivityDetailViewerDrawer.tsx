...
import { ActivityType } from '../../interfaces/activity.interfaces';
...
import { getTimeEntryById, saveTimeEntry } from '../../lib/actions/timeEntryActions';
...
import { TimeSheetStatus, ITimePeriodWithStatusView } from '../../interfaces/timeEntry.interfaces';
...
export function ActivityDetailViewerDrawer({
  ...
  // Fetch data and render appropriate component based on activity type
  useEffect(() => {
      ...
      try {
        ...
        switch(activityType) {
          case ActivityType.TICKET: {
            const ticketData = await getTicketById(activityId, currentUser);
            setContent(
              <div className="h-full">
                <TicketDetails initialTicket={ticketData} />
              </div>
            );
            break;
          }
